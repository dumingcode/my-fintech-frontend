...
export default {
    ...
    methods: {
        ...
        handleTargetPriceChange (val, index, key) {
            ...
            if (coverObj['gap']) {
                // 存储已补仓次数
                const myGridGap = getStore('myGridGap');
                if (myGridGap) {
                    const myGridGapJson = JSON.parse(myGridGap);
            ...
            if (coverObj['curLevel']) {
                // 存储成本价格键值对
                const myGridCurLevel = getStore('myGridCurLevel');
                if (myGridCurLevel) {
                    const myGridCurLevelJson = JSON.parse(myGridCurLevel);
