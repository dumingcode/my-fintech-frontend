...
export default {
    ...
    methods: {
        ...
        handleDel (val, index, delObj) {
            ...
            // 删除自选股已补仓次数
            const myStocksCoverTime = getStore('myStocksCoverTime');
            if (myStocksCoverTime) {
                const myStocksCoverTimeJson = JSON.parse(myStocksCoverTime);
                const delCoverObj = myStocksCoverTimeJson[delCode];
                if (delCoverObj >= 0) {
            ...
            // 删除自选股成本
            const myStocksCost = getStore('myStocksCost');
            if (myStocksCost) {
                const myStocksCostJson = JSON.parse(myStocksCost);
                const delCostObj = myStocksCostJson[delCode];
                if (delCostObj) {
        ...
        async refreshMyStock () {
            ...
            if (myStocksCost) {
                const myStocksJson = JSON.parse(myStocksCost);
